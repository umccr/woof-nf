...
discover_smlv_files <- function(s.base_dir) {
  ...
  # Combine, set render info, and return
  d.files <- dplyr::bind_rows(
    d.file_info_counts,
    d.file_info_comparisons
  )
  d.files$render <- d.files$exists[d.files$file_type=='count'] && any(d.files$exists[d.files$file_type %in% v.comparison_file_types])
  return(d.files)
}
...
v.smlv_files_render <- v.smlv_files[sapply(v.smlv_files, function(d) { any(d$render) })]
# Process data or create empty tibble
if (length(v.smlv_files_render) > 0) {
...
if (length(v.smlv_pcgr_files_exist) > 0) {
  ...
  d.smlv_pcgr_data <- dplyr::bind_rows(v.smlv_vcf_columns)[NULL, ]
}

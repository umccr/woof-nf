...
def entry():
    ...
    log.render(f'  s3_bucket: {args.s3_bucket if args.s3_bucket else "na"}')
    log.render(f'  resume: {args.resume}')
    log.render(f'  threads: {args.threads}')
    log.render(f'  docker: {args.docker}\n')
    ...
    # Execute pipeline
    workflow.run(inputs_fp, args.output_dir)
